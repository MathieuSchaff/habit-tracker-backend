...
export async function findValidSessionBySidHash(db: DB, sidHash: string) {
  ...
  const [row] = await db
    ...
    .from(sessions)
    .where(
      and(eq(sessions.sidHash, sidHash), isNull(sessions.revokedAt), gt(sessions.expiresAt, now))
    )
    .limit(1)
...
export async function revokeSession(db: DB, sidHash: string) {
  await db.update(sessions).set({ revokedAt: new Date() }).where(eq(sessions.sidHash, sidHash))
}
export async function updateLastSeen(db: DB, sidHash: string) {
  await db.update(sessions).set({ lastSeenAt: new Date() }).where(eq(sessions.sidHash, sidHash))
}
