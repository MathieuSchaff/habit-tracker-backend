  ...
  describe("Multiple sessions", () => {
    it("should allow multiple active sessions for same user", async () => {
      ...
      if (login1.success && login2.success && login3.success) {
        ...
        const session1 = await findValidSessionBySidHash(
          testDb,
          hashSid(login1.data.sid)
        );
        const session2 = await findValidSessionBySidHash(
          testDb,
          hashSid(login2.data.sid)
        );
        const session3 = await findValidSessionBySidHash(
          testDb,
          hashSid(login3.data.sid)
        );
        ...
        const session1After = await findValidSessionBySidHash(
          testDb,
          hashSid(login1.data.sid)
        );
        const session2After = await findValidSessionBySidHash(
          testDb,
          hashSid(login2.data.sid)
        );
