...
describe('User Product Routes (formerly Stock)', () => {
  let app: Hono<AppEnv>
  ...
  // ━━━ POST /user-products/:productId/stock-entries ━━━━━━━━━━━━━━━━━━━━━━━━

  describe('POST /user-products/:productId/stock-entries', () => {
    it('should create a stock entry and increment product_stock qty', async () => {
      ...
      const res = await authPost(app, `/user-products/${product.id}/stock-entries`, token, {
        qty: 3,
        pricePaidCents: 2500,
    ...
    it('should accumulate qty on repeated entries', async () => {
      ...
      await authPost(app, `/user-products/${product.id}/stock-entries`, token, {
        qty: 2,
    ...
    it('should reject unauthenticated request', async () => {
      const res = await app.request('/user-products/00000000-0000-0000-0000-000000000001/stock-entries', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    it('should only increment stock for the authenticated user', async () => {
      ...
      await authPost(app, `/user-products/${product.id}/stock-entries`, tokenToto, {
        qty: 3,
        purchasedAt: '2026-03-13',
      })

      // Alice should have no stock entries
      const res = await authGet(app, '/user-products/stock-entries', tokenAlice)
      const data = await res.json()
